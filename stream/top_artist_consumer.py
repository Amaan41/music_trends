...
# Aggregate: Count plays per artist per sliding window
agg_df = df_parsed.withWatermark("ts", "30 minutes") \
    .groupBy(window(col("ts"), "30 minutes", "5 minutes"), col("artist_name")) \
...
def write_to_mysql(batch_df, batch_id):
    ...
    if clear_table_flag:
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("DELETE FROM top_artist")
            conn.commit()
    ...
    # Apply ranking: Get top 1 artist in each window
    window_spec = Window.partitionBy("window").orderBy(col("play_count").desc())

    top_artists = batch_df.withColumn("rank", row_number().over(window_spec)) \
                          .filter(col("rank") == 1) \
                          .selectExpr(
...
query = agg_df.writeStream \
    ...
    .outputMode("update") \
    .option("checkpointLocation", "/tmp/spark_checkpoint/top_artist_count") \
    .start()
